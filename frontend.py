...
def search_command():
    list1.delete(0, END)
    for ele in Database.searchr(t_inp1.get(), t_inp2.get(), t_inp3.get(), t_inp4.get()):
        list1.insert(END, ele)

def addr_command():
    Database.insert(t_inp1.get(), t_inp2.get(), t_inp3.get(), t_inp4.get())
    list1.delete(0, END)
    view_command()

def delete_command():
    Database.delete_one(target_book[0])
    list1.delete(0, END)
    view_command()

def update_command():
    Database.updater(target_book[0] ,t_inp1.get(), t_inp2.get(), t_inp3.get(), t_inp4.get())
    list1.delete(0, END)
    view_command()
